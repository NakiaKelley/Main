...
Using ReportLab canvas for pixel-perfect control.

This is the v3.18 build script recovered from the original Claude.ai chat.
It is preserved here verbatim (with chat-formatting artifacts cleaned)
as the canonical baseline. Apply edits in build_corporate_v3_19.py instead
of modifying this file, so we always have a known-good baseline.
"""
...
def draw_page_footer(c, version="v3.18"):
    fy = MB + FTR_H - 4
    c.setStrokeColor(AMBER)
    c.setLineWidth(0.75)
    ...
    c.setFont('Poppins-Light', 6.5)
    vw = c.stringWidth(version, 'Poppins-Light', 6.5)
    c.drawString(W - MR - vw, fy - 10, version)
...
HORS_VEG = [
    ("Caprese Skewers", "Fresh mozzarella, grape tomato, basil, EVOO", "GF"),
...
HORS_BEEF_L = [
    ...
    ("Mini Beef Picadillo Empanadas", "Seasoned ground beef, olives, raisins", "DF"),
...
HORS_BEEF_R = [
    ...
    ("Mini BBQ Beef Empanadas", "Smoked beef, caramelized onion, salsa roja", "DF"),
...
SIG_POULTRY_R = [
    ...
    ("Turkey Meatloaf", "Classic glazed, mushroom gravy", "DF"),
]
SIG_BEEF_L = [
...
SIDES_SOUTH_L = [
    ...
    ("Candied Yams", "Brown sugar, butter, cinnamon, nutmeg", "GF"),
...
STATIONS_BYO_L = [
    ...
    ("Interactive Pasta Bar", "Penne & fettuccine, house tomato basil sauce, alfredo, grilled chicken, vegetables", ""),
]
STATIONS_BYO_R = [
    ("Nacho Bar", "Tortilla chips, nacho cheese, shredded chicken, refried beans, pico, jalapenos, guacamole, sour cream", "GF"),
    ("Avocado Bar", "Ripe avocado halves, smoked bacon, corn relish, cotija, habanero crema, pico de gallo", "GF"),
...
STATIONS_CHEF_R = [
    ("BBQ Sundae Bar", "Pulled chicken, brisket, or beans over mashed potatoes or mac & cheese, BBQ sauces, toppings", ""),
    ...
    ("Breakfast Taco Station", "Scrambled eggs, potatoes, turkey sausage, warm tortillas, salsa, cheese", "GF"),
]
STATIONS_CARVE_L = [
...
DES_CLASSIC_L = [
    ...
    ("Bread Pudding", "Bourbon sauce, golden raisins, whipped cream", ""),
]
DES_CLASSIC_R = [
    ("Mini Cheesecakes", "Assorted flavors, graham crust, fresh fruit", ""),
    ("Brownie Bites", "Fudgy dark chocolate, powdered sugar", ""),
    ("Assorted Cookie Platter", "Chocolate chip, sugar, snickerdoodle, oatmeal raisin", "Vegan"),
    ("Lemon Bars", "Shortbread crust, lemon curd, powdered sugar", ""),
    ("Chocolate Covered Strawberries", "Dark and milk chocolate dipped, individually boxed", "GF"),
]
...
class PageManager:
    ...
    def _start_page(self):
        ...
        draw_page_footer(self.c, self.version)
        self.y = BODY_T
    ...
    def cat(self, title, min_content_height=30):
        # Reserve space for the category header + at least one row of content
        # so the header never orphans at the bottom of a page.
...
def build_corporate(path, version="v3.18"):
    ...
    pm.two_col(CHARCUTERIE[:2], CHARCUTERIE[2:])
    pm.spacer(20)
...
if __name__ == '__main__':
    out = os.path.join(SCRIPT_DIR, 'Citizens_Corporate_Social_v3.18.pdf')
    build_corporate(out, version="v3.18")
    print("All done.")
